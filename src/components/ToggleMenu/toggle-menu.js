...
function ToggleMenu ({ children, className }) {
    ...
    if (stateToggle.openMenu) {
        mergeClassesList.push(classes['root__list--visible']);
    }

    const toggleMenuAction = function (e) {
        e.preventDefault();
        setStateToggle({
            openMenu: !stateToggle.openMenu
        });
    };

    return (
        <div className={mergeClasses.join(' ')}>
            <a href="#" onClick={toggleMenuAction} className={classes.root__button}>
            ...
            <ul className={mergeClassesList.join(' ')}>
                ...
                {children.map((listItem) => <li key={seed(listItem)} className={classes['root__list-item']}>{listItem}</li>)}
            </ul>
        </div>
    )
}
